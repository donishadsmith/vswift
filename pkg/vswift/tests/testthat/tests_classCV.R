...
test_that("k-fold CV with stratified", {
  data <- iris
  expect_no_error(result <- classCV(data = data, target = "Species", n_folds = 3, model_type = "ann", size = 10, stratified = TRUE, random_seed = 123))
})
...
test_that("test imputation and missing data", {
  ...
  # knn
  expect_warning(expect_warning(result <- classCV(data = data, target = "Species", split = 0.8, n_folds = 3, impute_method = "knn_impute", impute_args = list(neighbors = 5), model_type = "qda", stratified = TRUE)))
  
  # bag
  expect_warning(expect_warning(result <- classCV(data = data, target = "Species", split = 0.8, n_folds = 3, impute_method = "bag_impute", impute_args = list(trees = 5), model_type = "multinom", stratified = TRUE)))
  
  # complete cases only
  expect_warning(result <- classCV(data = data, target = "Species", split = 0.8, n_folds = 3, model_type = "decisiontree", stratified = TRUE))
})

test_that("test random seed", {
  
  data <- iris
  result_1 <- classCV(data = data, target = "Species", split = 0.8, n_folds = 3, model_type = "knn", random_seed = 123, stratified = TRUE, ks = 5)
  result_2 <- classCV(data = data, target = "Species", split = 0.8, n_folds = 3, model_type = "knn", random_seed = 123, stratified = TRUE, ks = 5)
