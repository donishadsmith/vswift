...
.clean_data <- function(data, missing_info, imputation_requested) {
  ...
  if (imputation_requested) {
    msg2 <- sprintf("%s labeled observations are missing data in one or more features and will be imputed",
                    missing_info$n_incomplete_labeled_data)
  } else {
    msg2 <- sprintf("dropping %s labeled observations with one or more missing features",
