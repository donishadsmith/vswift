...
test_that("test pr curve", {
  ...
  pr_output <- results$pr_curve(path = getwd())
  check_png()
  expect_true(length(pr_output$get_model()) == 13)
  check_metrics(pr_output, "pr")

  # With specified thresholds
  pr_output <- results$pr_curve(path = getwd(), thresholds = seq(0, 0.9, 0.1))
  check_png()
  expect_true(length(pr_output$get_model()) == 13)
  check_metrics(pr_output, "pr")
})
...
test_that("test equivalence with standardizing", {
  ...
  output2 <- result2$pr_curve(data = data)
  ...
  for (fold in names(svm1$cv)) {
    for (i in names(svm1$cv[[fold]])) {
      if (i == "metrics") {
        expect_true(all(svm1$cv[[fold]]$metrics$recall == svm2$cv[[fold]]$metrics$recall))
        expect_true(all(svm1$cv[[fold]]$metrics$precision == svm2$cv[[fold]]$metrics$precision))
      } else {
