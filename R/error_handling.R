# Helper function for class_cv to check if inputs are valid
.error_handling <- function(data, formula = NULL, target = NULL, predictors = NULL, models = NULL,
                            ...
                            parallel_configs = NULL, create_data = NULL) {
  ...
  .stop_execution(train_params, model_params)

  if (!is.null(train_params$n_folds) && train_params$n_folds <= 2) {
    stop("`train_params$n_folds` must greater than 2")
  }
  ...
  if (inherits(c(formula, target), "NULL")) {
    stop("either `formula` or `target` must be specified")
  }
  ...
  if (any(c("regularized_logistic", "regularized_multinomial") %in% models) && !is.null(model_params$rule)) {
    ...
    if (length(intersect_char) == 0) {
      stop("'min' and '1se' are the only valid options for `model_params$rule`")
    }
  }
...
.stop_execution <- function(train_params, model_params) {
...
.check_binary_models <- function(data, formula, target, models, model_params) {
  ...
  if (!is.null(model_params$threshold)) {
    ...
    if (!valid_threshold) {
      stop("`model_params$threshold` must a numeric value from 0 to 1")
    }
  }
}
...
.check_map_args <- function(model_params, valid_models, error_msg) {
  ...
  if (!is.null(model_params$map_args)) {
    .check_args(model_params = model_params, caller = "model")
  }
}
