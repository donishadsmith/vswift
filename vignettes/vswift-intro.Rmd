...
### The main function of the **vswift** package is `class_cv`.
```{r}
result1 <- class_cv(
...
```{r}
result3$print(models = "knn")
```

### `plot` can be used to visualize performance metrics.
```{r}
result3$plot(models = "knn", split = FALSE, cv = TRUE, class_names = c("setosa", "virginica"), metrics = "precision")
```

### `roc_curve` can be used to generate ROC curves and ROC-AUC scores for binary classification targets. The model and preprocessed dataset must be saved.
```{r}
data <- iris
