...
# Perform a train-test split with five folds using stratified sampling with K-Nearest Neighbors while also specifying an additional argument for nnet to specify the number of neighbors to consider
result2 <- classCV(
...
# Perform a train-test split with an 80% training set and five folds with stratified sampling LDA and nnet. Also specify an argument for nnet and save the models for lda and nnet.

args <- list("nnet" = list(size = 5))
...
```{r}
print(result3, models = "nnet")
```


### `plot` function can be used to visualize performance metrics.
```{r}
plot(result3, models = "nnet", split = FALSE, cv = TRUE, class_names = c("setosa", "virginica"), metrics = "precision")
```
